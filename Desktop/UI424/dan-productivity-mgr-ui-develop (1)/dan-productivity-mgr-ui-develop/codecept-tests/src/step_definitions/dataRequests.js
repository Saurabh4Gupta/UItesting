...
Given(/^User will click on "([^"]*)" button and will see heading as "([^"]*)"$/, async function(button, Title) {
 await homepage.clickOnButton(button);
...
Then(/^User will click on "([^"]*)" button and validation for respective fields "([^"]*)" will be shown\.$/, function(button, text) {
  homepage.clickOnButton(button);
  I.wait(3);
  homepage.verifyDivText(text);
});
// Scenario2//
...
Given(/^User will select filer and will select "([^"]*)" again$/,async function(value) {

  await homepage.filter1(value);
});
