...
const DataField = (props) => {
  ...
  const { data: completeListData, refetch: refetchCompletedata } = useQuery(
    GET_DATA_LIST,
    {
      variables: { data: { clientCode, marketCode: market.value, complete: true } },
      fetchPolicy: 'network-only',
    },
  );
  ...
  useEffect(() => {
    if (completeListData) {
      const {
        data: { dataList, totalCount },
      } = completeListData.getDataList;
      const parsedData = parsedDataList(dataList, marketOptions, name);
      setCompleteDataList({ dataList: parsedData, totalCount });
    }
  }, [completeListData]);
      ...
      <Box mb="200px">
        ...
        >
          ...
          <DataList
            ...
            handleModal={handleModal}
            refetch={refetch}
            refetchCompletedata={refetchCompletedata}
            clientCode={clientCode}
          />
        </PageController>
      </Box>
