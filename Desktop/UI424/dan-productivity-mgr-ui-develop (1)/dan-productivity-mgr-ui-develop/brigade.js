...
async function getValues(teamEnv, project) {
  ...
  const values = {
    ...
    apollo_graphql_uri: `https://${
      project.secrets[`${teamEnv}_sharedEnvGA`]
    }-kong-proxy.az.${
      project.secrets[`${teamEnv}-target_name`]
    }.gdpdentsu.net/prodman`,
    apollo_subscription_uri: `wss://${
      project.secrets[`${teamEnv}_sharedEnvGA`]
    }-kong-proxy.az.${
      project.secrets[`${teamEnv}-target_name`]
    }.gdpdentsu.net/prodman`,
    ...
    flagit_is_embedded_app:
      project.secrets[`${teamEnv}_flagit_is_embedded_app`],
    use_method_to_fetch_perms:
      project.secrets[`${teamEnv}_use_method_to_fetch_perms`],
    um_graphql_url: `https://shared01-${env1}-g1ga-kong-proxy.az.eu-az-${env1}-wal.gdpdentsu.net/user`,
    instrumentation_key: project.secrets[`${teamEnv}_instrumentation_key`],
    ingress: {
      class: 'nginx',
      enabled: true,
      hosts: [
        `${teamEnv}-${project.secrets.app_name}.az.${
          project.secrets[`${teamEnv}-target_name`]
        }.gdpdentsu.net`,
      ],
      tls: [
        {
          secretName: `${teamEnv}-${project.secrets.app_name}.az.${
            project.secrets[`${teamEnv}-target_name`]
          }.gdpdentsu.net`,
          hosts: [
            `${teamEnv}-${project.secrets.app_name}.az.${
    ...
    values.ingress.tls = [
      {
        ...
        hosts: [`productivity-ui.wal.${env1}.az.eu.mediaecosystem.io`],
      },
    ];
  }
  ...
  if (devops.MEUtilities.isRouteToHotfix(teamEnv)) {
    values.apollo_graphql_uri = `https://${
      project.secrets[`${teamEnv}_sharedEnvGA`]
    }-kong-proxy.az.${
      project.secrets[`${teamEnv}-target_name`]
    }.gdpdentsu.net/prodman`;
    values.apollo_subscription_uri = `wss://${
      project.secrets[`${teamEnv}_sharedEnvGA`]
    }-kong-proxy.az.${
      project.secrets[`${teamEnv}-target_name`]
    }.gdpdentsu.net/prodman`;
